...
def test_parse_react_thermo_data():
    ...
    nqe.run_instanton_post_process(
        directory, process_type="reactant", temperature=temperature
    )
...
def test_parse_ts_thermo_data():
    ...
    nqe.run_instanton_post_process(
        directory, process_type="TS", temperature=temperature
    )
...
def test_parse_inst_thermo_data():
    ...
    nqe.run_instanton_post_process(
        directory, process_type="instanton", temperature=temperature, n_beads=n_beads
    )
...
def test_calc_instanton_kappa_bead():
    ...
    nqe.run_instanton_post_process(
        directory_ts, process_type="TS", temperature=temperature
    )
...
def test_ch4hcbe_end_to_end():
    ...
    rate = nqe.calc_forward_rate(
        directory_phonon_react, directory_ts, temperature, filter_list=n_atoms - 1
    )

    print(f"Reaction rate = {rate}", flush=True)
    # assert np.allclose(rate, 1.0416486358380245e-08, rtol=1e-3)

    kappa = nqe.calc_kappa_full(
