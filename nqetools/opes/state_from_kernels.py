...
parser.add_argument(
    ...
    type=str,
    default="KERNELS",
    help="the kernels file name, with the deposited kernels",
)
parser.add_argument(
    "--outfile",
    "-o",
...
with open(filename) as f:
    ...
    for i in range(ncv):
        ...
        if line.split()[3 + ncv + i] != "sigma_" + cvname[i]:
            sys.exit(
                error
                % (f' expected "sigma_{cvname[i]}" instead of "{line.split()[4 + i]}"')
            )
    ...
    if line.split()[3] == "OPES_METAD_kernels":
        ...
        sys.exit(
            error
            % (
                " this script only works with OPES_METAD or OPES_METAD_EXPLORE KERNELS files"
            )
        )
        ...
        for i in range(ncv):
            if line.split()[2] == "min_" + cvname[i]:
                ...
                if line.split()[2] != "max_" + cvname[i]:
                    sys.exit(
                        error % (" periodic CVs should have both min and max value!")
                    )
                periodic[i] += line.split()[3]
        line = f.readline()
...
for i in range(ncv):
    if cvname[i].find(".") != -1:
        ...
        if (suffix[i] == ".x" or suffix[i] == ".y" or suffix[i] == ".z") and periodic[
            i
        ] == "NO":
...
cmd.wait()
print(
    " +++ IMPORTANT: do not use the obtained STATE file for restarting a simulation +++"
)
