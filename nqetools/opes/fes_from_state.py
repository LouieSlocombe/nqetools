...
if len(fields_pos) > 1:
    ...
    if all_stored:
        ...
        print(
            "  -> only the last one will be printed. use --all_stored to instead print them all"
        )
...
for n in range(len(fields_pos) - 1):
    ...
    if len(data[l, :]) == 6:
        ...
        sys.exit(
            ' wrong number of FIELDS in file "'
            + filename
            + '": only 1 or 2 dimensional bias are supported'
        )
    action = data[l + 1, 3]
    if action == "OPES_METAD_state":
        ...
        sys.exit(
            " This script works only with OPES_METAD_state and OPES_METAD_EXPLORE_state"
        )
    ...
    if (
        period_x == grid_cv_x[-1] - grid_cv_x[0]
    ...
    if not dim2:
        ...
        for i in range(grid_bin_x):
            ...
            kernels_i = height * (
                np.maximum(np.exp(-0.5 * dist_x * dist_x) - val_at_cutoff, 0)
            )
        ...
        for i in range(grid_bin_y):
            ...
            for j in range(grid_bin_x):
                ...
                kernels_ij = height * (
                    np.maximum(
                        np.exp(-0.5 * (dist_x**2 + dist_y**2)) - val_at_cutoff, 0
                    )
                )
